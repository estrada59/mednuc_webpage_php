<html>

<head>
    <?php include 'meta_servicios.php';?>
   
    <meta name="description" content="La gammagrafía de perfusión pulmonar es un método diagnóstico de imagen que estudia la distribución del flujo sanguíneo arterial en los pulmones.">

    <title>Medicina Nuclear</title>
</head>

<body class="sticky-header header-classic footer-parallax">

    <div id="main-container">

        <?php include_once "header.php"; ?>

        <!-- PAGE CONTENT -->
        <div id="page-content">

            <div id="page-header" class="parallax" data-stellar-background-ratio="0.3"
                 style="background-image: url(images/backgrounds/page-header-3.webp);">

                <div class="container">
                    <div class="row">
                        <div class="col-md-12">

                            <h1>Gammagrama Pulmonar Perfosorio y Ventilatorio</h1>

                        </div><!-- col -->
                    </div><!-- row -->
                </div><!-- container -->

            </div><!-- page-header -->

            <div class="container">
                <div class="row">
                    <div class="col-lg-5">

                        <p class="text-justify mb-5" ><img src="images/services/Gammagrama_Pulmonar.webp" alt="Gammagrama Pulmonar"></p>
                   

                    </div><!-- col -->
                    <div class="col-lg-7 col-xl-6 ml-auto">

                        <!--<h2 class="mb-0">Chris Martin</h2>
                        <p class="text-uppercase text-default-color">Neurologist</p>
                       <br>-->
                       <br>

                        <p>La gammagrafía pulmonar permite estudiar el intercambio gaseoso que se lleva acabo en los pulmones. En el estudio ventilatorio visualizamos la ausencia de llegada de aire a determinadas áreas pulmonares. Y en la perfusoria detectamos en que zonas del tejido pulmonar no reciben aporte sanguíneo.</p>

                        <h4></h4>
                        
                       
                    </div><!-- col -->
                </div><!-- row -->
    
                
            </div><!-- container -->

            <?php 
                include_once 'seccion_experiencia.php';
            ?>

            <div class="container">
                <div class="row">
                    <div class="col-md-7">
                        <p>Este estudio esta indicado para de diversos padecimientos como por ejemplo:</p>
                        <ul>
                            <li>Alteraciones de la perfusión posterior a COVID-19.</li>
                            <li>EPOC.</li>
                            <li>Insuficiencia respiratoria del adulto.</li>
                            <li>Secuelas de neumonías o bronquitis y bronquiolitis en niños y adultos como el síndrome de Swyer-James.</li>
                            <li>Embolia pulmonar.</li>
                        </ul>
                    </div><!-- col -->

                    <div class="col-md-5">
                        <p class="text-center mb-5"><img src="images/services/Gammagrama_Pulmonar.webp" alt="Gammagrama Pulmonar"></p>
                    </div><!-- col -->
                </div><!-- row -->
            </div><!-- container -->

            <section class="full-section dark-section parallax" id="section-6" data-stellar-background-ratio="0.3">

                <div class="full-section-overlay-color"></div>

                <div class="full-section-container">

                    <div class="container">
                        <div class="row">
                            <div class="col-md-12">

                                <h1 class="text-center">Estamos aquí para ti</h1>

                            </div><!-- col -->
                        </div><!-- row -->
                    </div><!-- container -->

                    <br>

                    <div class="container">
                        <div class="row">
                            <div class="col-lg-8 col-md-10 ml-auto mr-auto">

                                <p class="text-center">En Medicina Nuclear encontrará un personal altamente calificado para poder brindarle este servicio.</p>

                            </div><!-- col -->
                        </div><!-- row -->
                    </div><!-- container -->

                </div><!-- full-section-container -->
            </section><!-- full-section -->

            

        </div><!-- PAGE CONTENT -->


        <?php include_once "footer.php"; ?>

    </div><!-- MAIN CONTAINER -->

    <!-- SCROLL UP -->
    <a id="scroll-up" class="waves fa fa-angle-up"></a>

    <a id="scroll-up-what" class="waves fa fa-whatsapp" ></a>

    <?php include_once "script.php"; ?>

</body>

</html>
    


